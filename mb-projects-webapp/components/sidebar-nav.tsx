...
export function SidebarNav({ className, initialRole, ...props }: SidebarNavProps & { initialRole?: string | string[] | null }) {
    const pathname = usePathname();
    const router = useRouter();
    const supabase = createClient();
    const [userRole, setUserRole] = useState<string | string[] | null>(initialRole || null);
    const [mounted, setMounted] = useState(!!initialRole);
    ...
    useEffect(() => {
        ...
        async function fetchRole() {
            if (userRole) return; // Already have role from server
